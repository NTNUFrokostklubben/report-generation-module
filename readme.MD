# SKAVL report generation module

Module for generating reports on anomalies detected in the SKAVL anomaly detection module.
...
Install the conda environment from the lockfile (skavl-report should be replaced with whatever environment name you want to use in conda)
```shell
conda-lock install -n skavl-report .\conda-lock.yml
```
...
If the environment exists, activate it (replace skavl-report with the actual env name)
```shell
conda activate skavl-report
```
...
```
https://ntnufrokostklubben.github.io/report-generation-module/
