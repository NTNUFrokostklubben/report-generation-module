...
def create_report_classified(report_set: ReportSet):
    """
    ...
    project_report = (report_set.project_meta_data.project_name.replace(" ", "-") + "-classified-report.pdf")
    html.write_pdf(_REPORT_DIR / project_report, stylesheets=[css])

    response = rp2.ReportGenerationResponse()
    response.report_url = str(_TEMPLATE_DIR / project_report)
